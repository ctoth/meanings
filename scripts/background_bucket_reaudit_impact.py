"""Compute the A/B impact of the BG Phase 2 background-bucket rules.

Mirrors `scripts/artifact_bucket_reaudit_impact.py` but compares the
post-BG-rebuild pressure table against the pre-BG snapshot. Reuses the
validator's closure-fixpoint and classification logic.
"""
...
def background_share(counts: dict[str, int]) -> tuple[float, int, int]:
    bg = counts.get("background", 0)
    non_truncated_total = sum(counts.values()) - counts.get("graph_data", 0)
    if non_truncated_total <= 0:
        return 0.0, bg, 0
    return bg / non_truncated_total, bg, non_truncated_total
            ...
            rows.append(
                {
                    ...
                    "p2_seed": post_row.get("p2_seed", ""),
                    "frequency_post": post_row.get("frequency", ""),
                    "age_of_acquisition_post": post_row.get("age_of_acquisition", ""),
                    "concreteness_post": post_row.get("concreteness", ""),
                }
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute background-bucket re-audit impact.")
    parser.add_argument(
        "--pressure-table-pre", type=Path, default=Path("data/kernel-pressure-table.bg-pre.csv")
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report", type=Path, default=Path("reports/background-bucket-reaudit-impact.md")
    )
    parser.add_argument(
        "--json", type=Path, default=Path("reports/background-bucket-reaudit-impact.json")
    )
    parser.add_argument(
        "--per-sense-csv",
        type=Path,
        default=Path("reports/background-bucket-reaudit-impact-per-sense.csv"),
    )
    ...
    bg_pre, bg_count_pre, _ = background_share(counts_pre_le_200)
    bg_post, bg_count_post, _ = background_share(counts_post_le_200)
    ...
    payload = {
        "schema_version": 1,
        "artifact_id": "background-bucket-reaudit-impact",
        "definition": "A/B impact report for BG Phase 4 of background-bucket-reaudit-workstream",
        "inputs": {
        ...
        "augmented_layer": {
            "pre_size": len(augmented_layer_pre),
            "post_size": len(augmented_layer_post),
        ...
        "closure_size_le_200": {
            ...
            "closure_rate_delta_pp": (rate_post - rate_pre) * 100,
            "artifact_share_pre": art_pre,
            "artifact_share_post": art_post,
            "artifact_share_delta_pp": (art_post - art_pre) * 100,
            "background_share_pre": bg_pre,
            "background_share_post": bg_post,
            "background_share_delta_pp": (bg_post - bg_pre) * 100,
        },
        "all_targets": {
    ...
    lines: list[str] = []
    lines.append("# Background Bucket Re-audit Impact")
    lines.append("")
    lines.append("BG Phase 4 A/B diff for `reports/background-bucket-reaudit-workstream.md`.")
    lines.append("")
    lines.append("## Inputs")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"- Pre size: `{len(augmented_layer_pre)}`")
    lines.append(f"- Post size: `{len(augmented_layer_post)}`")
    added = sorted(set(augmented_layer_post) - set(augmented_layer_pre))
    removed = sorted(set(augmented_layer_pre) - set(augmented_layer_post))
    lines.append(f"- Added: `{len(added)}` ICs")
    ...
    lines.append(f"- Artifact share delta: `{(art_post - art_pre) * 100:+.2f} pp`")
    lines.append(f"- Background share pre: `{bg_pre:.4f}` (`{bg_count_pre}`)")
    lines.append(f"- Background share post: `{bg_post:.4f}` (`{bg_count_post}`)")
    lines.append(f"- Background share delta: `{(bg_post - bg_pre) * 100:+.2f} pp`")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Top 60 Promoted ICs")
    lines.append("")
    promoted = [
        r for r in migration_rows if r["post_pressure_bucket"] == "base_promotable_terminal_common"
    ]
    promoted.sort(key=lambda r: -float(r["frequency_post"] or 0))
    sample_rows = [
        {
            "ic_id": r["ic_id"],
            "primary_alias": r["primary_alias"],
            "pre_pressure_bucket": r["pre_pressure_bucket"],
            "frequency_post": r["frequency_post"],
            "age_of_acquisition_post": r["age_of_acquisition_post"],
            "concreteness_post": r["concreteness_post"],
        }
        for r in promoted[:60]
    ]
    lines.extend(
        render_md_table(
            sample_rows,
        ...
        json.dumps(
            {
                ...
                "artifact_share_delta_pp": (art_post - art_pre) * 100,
                "background_share_pre_le_200": bg_pre,
                "background_share_post_le_200": bg_post,
                "background_share_delta_pp": (bg_post - bg_pre) * 100,
                "closed_pre_aug": eval_pre_aug["all_counts"].get("closed", 0),
                "closed_post_aug": eval_post_aug["all_counts"].get("closed", 0),
                "mgy_pre": mgy_pre,
